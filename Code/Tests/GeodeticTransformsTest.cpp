...
namespace UnitTest
{
    ...
    class GeodeticTransformsTest : public AllocatorsTestFixture {
    };

    TEST_F(GeodeticTransformsTest, WGS84ToECEF) {
        ...
        for (const auto& [input, goldResult] : input_gold_set) {
            AZ::Vector3 result = ROS2::Utilities::GeodeticTransforms::WGS84ToECEF(input);
    ...
    TEST_F(GeodeticTransformsTest, ECEFToENU) {
        ...
        for (const auto& [input, refWGS84, goldResult] : input_gold_set) {
            AZ::Vector3 result = ROS2::Utilities::GeodeticTransforms::ECEFToENU(refWGS84, input);
            EXPECT_NEAR(result.GetX(), goldResult.GetX(), 1.0f);
            EXPECT_NEAR(result.GetY(), goldResult.GetY(), 1.0f);
            EXPECT_NEAR(result.GetZ(), goldResult.GetZ(), 1.0f);
        }
    ...
    TEST_F(GeodeticTransformsTest, ENUToECEF) {
        const std::vector<std::tuple<AZ::Vector3, AZ::Vector3, AZ::Vector3>> input_gold_set = {
        ...
        for (const auto& [ input, refWGS84, goldResult] : input_gold_set) {
            AZ::Vector3 result = ROS2::Utilities::GeodeticTransforms::ENUToECEF(refWGS84, input);
            EXPECT_NEAR(result.GetX(), goldResult.GetX(), 1.0f);
            EXPECT_NEAR(result.GetY(), goldResult.GetY(), 1.0f);
            EXPECT_NEAR(result.GetZ(), goldResult.GetZ(), 1.0f);
        }
    ...
    TEST_F(GeodeticTransformsTest, ECEFToWSG84) {
        const std::vector<std::pair<AZ::Vector3, AZ::Vector3>> input_gold_set = {
        ...
        for (const auto& [input, goldResult] : input_gold_set) {
            AZ::Vector3 result = ROS2::Utilities::GeodeticTransforms::ECEFToWGS84(input);
